...
class Attention(nn.Module):
    ...
    def execute(self, x):
        ...
        x = (attn @ v).transpose(0, 2, 1, 3).flatten(2)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.is_training():
            self.pos_embed.start_grad()
            self.cls_token.start_grad()
...
class DistilledVisionTransformer(VisionTransformer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dist_token = jt.zeros((1, 1, self.embed_dim))
        num_patches = self.patch_embed.num_patches
        self.pos_embed = jt.zeros((1, num_patches + 2, self.embed_dim))
        self.head_dist = nn.Linear(self.embed_dim, self.num_classes) if self.num_classes > 0 else nn.Identity()

        self.dist_token = trunc_normal_(self.dist_token, std=.02)
    ...
    def forward_features(self, x):
        ...
        x = jt.concat((cls_tokens, dist_token, x), dim=1)
