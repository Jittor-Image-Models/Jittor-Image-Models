    ...
    def load_pretrained(self, checkpoint_path, prefix='resnet/'):
        ...
        with jt.no_grad():
            ...
            if self.stem.conv.weight.shape[1] == 1:
                self.stem.conv.weight.copy(stem_conv_w.sum(dim=1, keepdim=True))
                # FIXME handle > 3 in_chans?
            else:
                self.stem.conv.weight.copy(stem_conv_w)

            self.norm.weight.copy(tf2th(weights[f'{prefix}group_norm/gamma']))
            self.norm.bias.copy(tf2th(weights[f'{prefix}group_norm/beta']))
            self.head.fc.weight.copy(tf2th(weights[f'{prefix}head/conv2d/kernel']))
            self.head.fc.bias.copy(tf2th(weights[f'{prefix}head/conv2d/bias']))
            for i, (sname, stage) in enumerate(named_children(self.stages)):
                for j, (bname, block) in enumerate(named_children(stage.blocks)):
                    convname = 'standardized_conv2d'
                    block_prefix = f'{prefix}block{i + 1}/unit{j + 1:02d}/'
                    block.conv1.weight.copy(tf2th(weights[f'{block_prefix}a/{convname}/kernel']))
                    block.conv2.weight.copy(tf2th(weights[f'{block_prefix}b/{convname}/kernel']))
                    block.conv3.weight.copy(tf2th(weights[f'{block_prefix}c/{convname}/kernel']))
                    block.norm1.weight.copy(tf2th(weights[f'{block_prefix}a/group_norm/gamma']))
                    block.norm2.weight.copy(tf2th(weights[f'{block_prefix}b/group_norm/gamma']))
                    block.norm3.weight.copy(tf2th(weights[f'{block_prefix}c/group_norm/gamma']))
                    block.norm1.bias.copy(tf2th(weights[f'{block_prefix}a/group_norm/beta']))
                    block.norm2.bias.copy(tf2th(weights[f'{block_prefix}b/group_norm/beta']))
                    block.norm3.bias.copy(tf2th(weights[f'{block_prefix}c/group_norm/beta']))
                    if block.downsample is not None:
                        w = weights[f'{block_prefix}a/proj/{convname}/kernel']
                        block.downsample.conv.weight.copy(tf2th(w))
